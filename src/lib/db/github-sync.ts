import { exportBytes, importBytes, save } from "./index";
import { authStore } from "$lib/stores/auth.svelte";

const SYNC_META_KEY = "github-sync-meta";
const DEFAULT_PATH = "stronglifts.sqlite";

interface SyncMeta {
  repo: string;
  path: string;
  lastSha: string | null;
  lastPushAt: string | null;
  lastPullAt: string | null;
}

function getMeta(): SyncMeta {
  if (typeof localStorage === "undefined")
    return {
      repo: "",
      path: DEFAULT_PATH,
      lastSha: null,
      lastPushAt: null,
      lastPullAt: null,
    };
  try {
    const raw = localStorage.getItem(SYNC_META_KEY);
    if (raw) return JSON.parse(raw);
  } catch {}
  return {
    repo: "",
    path: DEFAULT_PATH,
    lastSha: null,
    lastPushAt: null,
    lastPullAt: null,
  };
}

function setMeta(meta: SyncMeta) {
  if (typeof localStorage !== "undefined") {
    localStorage.setItem(SYNC_META_KEY, JSON.stringify(meta));
  }
}

export function getSyncStatus() {
  const meta = getMeta();
  return {
    configured: !!meta.repo && !!authStore.token,
    repo: meta.repo,
    path: meta.path,
    lastPushAt: meta.lastPushAt,
    lastPullAt: meta.lastPullAt,
  };
}

export function configure(repo: string, path?: string) {
  const meta = getMeta();
  meta.repo = repo;
  if (path) meta.path = path;
  setMeta(meta);
}

function headers(): Record<string, string> {
  return {
    Authorization: `Bearer ${authStore.token}`,
    Accept: "application/vnd.github+json",
    "X-GitHub-Api-Version": "2022-11-28",
  };
}

export async function push(): Promise<{
  success: boolean;
  error?: string;
}> {
  const meta = getMeta();
  if (!meta.repo || !authStore.token) {
    return { success: false, error: "Not configured" };
  }

  try {
    // Ensure latest data is persisted
    await save();
    const bytes = exportBytes();
    const base64 = uint8ToBase64(bytes);

    // Get current file SHA (needed for updates)
    let sha: string | undefined;
    try {
      const res = await fetch(
        `https://api.github.com/repos/${meta.repo}/contents/${meta.path}`,
        { headers: headers() },
      );
      if (res.ok) {
        const data = await res.json();
        sha = data.sha;
      }
    } catch {
      // File doesn't exist yet, that's fine
    }

    // Push the file
    const body: Record<string, unknown> = {
      message: `sync: ${new Date().toISOString()}`,
      content: base64,
    };
    if (sha) body.sha = sha;

    const res = await fetch(
      `https://api.github.com/repos/${meta.repo}/contents/${meta.path}`,
      {
        method: "PUT",
        headers: { ...headers(), "Content-Type": "application/json" },
        body: JSON.stringify(body),
      },
    );

    if (!res.ok) {
      const err = await res.text();
      return { success: false, error: `GitHub API: ${res.status} ${err}` };
    }

    const result = await res.json();
    meta.lastSha = result.content.sha;
    meta.lastPushAt = new Date().toISOString();
    setMeta(meta);

    return { success: true };
  } catch (e) {
    return { success: false, error: String(e) };
  }
}

export async function pull(): Promise<{
  success: boolean;
  updated: boolean;
  error?: string;
}> {
  const meta = getMeta();
  if (!meta.repo || !authStore.token) {
    return { success: false, updated: false, error: "Not configured" };
  }

  try {
    // Check if file exists and get its SHA
    const metaRes = await fetch(
      `https://api.github.com/repos/${meta.repo}/contents/${meta.path}`,
      { headers: headers() },
    );

    if (!metaRes.ok) {
      if (metaRes.status === 404) {
        return { success: true, updated: false }; // No remote DB yet
      }
      return {
        success: false,
        updated: false,
        error: `GitHub API: ${metaRes.status}`,
      };
    }

    const fileData = await metaRes.json();

    // Skip if SHA hasn't changed
    if (fileData.sha === meta.lastSha) {
      return { success: true, updated: false };
    }

    // Download the raw file
    const rawRes = await fetch(fileData.download_url);
    if (!rawRes.ok) {
      return {
        success: false,
        updated: false,
        error: "Failed to download DB file",
      };
    }

    const buffer = await rawRes.arrayBuffer();
    const bytes = new Uint8Array(buffer);

    await importBytes(bytes);

    meta.lastSha = fileData.sha;
    meta.lastPullAt = new Date().toISOString();
    setMeta(meta);

    return { success: true, updated: true };
  } catch (e) {
    return { success: false, updated: false, error: String(e) };
  }
}

// Auto-sync: debounced push
let pushTimer: ReturnType<typeof setTimeout> | null = null;
const PUSH_DELAY_MS = 30_000;

export function schedulePush() {
  if (pushTimer) clearTimeout(pushTimer);
  const status = getSyncStatus();
  if (!status.configured) return;
  pushTimer = setTimeout(() => {
    push();
    pushTimer = null;
  }, PUSH_DELAY_MS);
}

export function setupAutoSync() {
  if (typeof document === "undefined") return;

  // Push when tab becomes hidden
  document.addEventListener("visibilitychange", () => {
    if (document.visibilityState === "hidden") {
      const status = getSyncStatus();
      if (status.configured && pushTimer) {
        clearTimeout(pushTimer);
        pushTimer = null;
        push();
      }
    }
  });
}

// Helpers
function uint8ToBase64(bytes: Uint8Array): string {
  let binary = "";
  for (let i = 0; i < bytes.length; i++) {
    binary += String.fromCharCode(bytes[i]);
  }
  return btoa(binary);
}
